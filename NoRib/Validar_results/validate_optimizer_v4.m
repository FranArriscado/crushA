...
% Only input needed: the .mat filename from results/NoRib/
matFile = 'square_maxCF_maxcfsw2_02-04_(v20).mat';
...
% Read optimizer quantities directly from .mat
pythonForceFinal = double(data.forceFinal);
...
diff_N   = forceTotal - pythonForceFinal;
diff_pct = 100 * diff_N / pythonForceFinal;
...
ts  = datestr(now, 31);
[~, matName, matExt] = fileparts(matFile);
fprintf(fid, '%s,%s,%.2f,%.1f,%.2f,%.2f,%.2f,%.4f\n', ...
    ts, [matName matExt], thickness_val, min_roc, ...
    forceTotal, pythonForceFinal, diff_N, diff_pct);
fclose(fid);
...
% exportName   = regexprep(matName, '_\d+(?:\.\d+)?k?_\d{2}-\d{2}_\(v\d+\)$', '');
% if strcmp(exportName, matName)
%     exportName = regexprep(matName, '_\d+(?:\.\d+)?k?_\d{2}-\d{2}$', '');
% end
% if isempty(exportName)
%     exportName = matName;
% end
% 
% save(fullfile(cfg.validation, exportName), ...
