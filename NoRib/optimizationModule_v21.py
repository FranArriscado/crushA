...
# ---- Project paths ---- #
NORIB_ROOT = Path(__file__).resolve().parent
DATA_DIR = NORIB_ROOT / "data"
...
thickness = 2.4                       # [mm]
dataFileName = 'partition_data_layer36_z207.6_fixed_uniform.mat'
...
box_width = 225                      # [mm] box extent in X
box_height = 175                     # [mm] box extent in Y
...
def closed_loop_query_params(
    ...
    return (s_nodes.unsqueeze(1) + (s_next - s_nodes).unsqueeze(1) * alpha).reshape(-1)
...
class crushOptimizerModel(nn.Module):
    ...
    def _reconstruct_ordered_nodes(self) -> torch.Tensor:
        ...
        return torch.stack([X, Y, Z], dim=1) + self.centroid

    def _build_spline_cache(self, orderedNodes: torch.Tensor):
    ...
    def smoothed_curve_points(self, samples_per_edge: int = 1) -> torch.Tensor:
        if self._last_spline_cache is None:
            orderedNodes = self._reconstruct_ordered_nodes()
            cache = self._build_spline_cache(orderedNodes)
        else:
            cache = self._last_spline_cache

        sQuery = closed_loop_query_params(
            cache['s'], self.overlapCount, samples_per_edge=samples_per_edge)
        return eval_spline_points(cache['s'], cache['g'], cache['M'], cache['h'], sQuery)

    def forward(self):
        orderedNodes = self._reconstruct_ordered_nodes()
        spline_cache = self._build_spline_cache(orderedNodes)
        splineDer1, splineDer2 = query_spline_derivatives(
            spline_cache['s'], spline_cache['g'], spline_cache['M'],
            spline_cache['h'], self.overlapCount, samples_per_edge=1)
        # RoC calculation
...
def _box_penalty(model, box_dims):
    ...
    The force model operates on the smoothed spline, so the box constraint
    must be enforced on that same continuous curve instead of on the nodes
    alone.
    """
    curve_points = model.smoothed_curve_points(
        samples_per_edge=box_constraint_samples_per_edge) - model.centroid
    X = curve_points[:, 0]
    Y = curve_points[:, 1]
    ...
    ) / float(box_constraint_samples_per_edge)
...
def compute_loss(mode, force_total, perimeter, model,
    ...
    if mode == 'maxCF':
        ...
        raise ValueError(
            f"Unknown mode '{mode}'. "
            f"Use 'maxCF', 'minP', 'maxFPP' (legacy alias: 'maxSEA'), or 'boxMaxCF'.")
...
# ---- Resolve input path and build output filenames ---- #
...
# Use only the input file basename for output naming so absolute input paths
# cannot override the configured results directory on Windows.
...
with torch.no_grad():
        ...
        force_per_perimeter_initial = forceTotal_initial / perimeter_initial
...
if run_mode in ('maxCF', 'boxMaxCF'):
    ...
    objective_scale = force_per_perimeter_initial.item()
...
with torch.no_grad():
    ...
    initial_radial_ok = satisfies_radial_constraint(model, run_mode, max_expansion_ratio)
...
for epoch in range(n_steps):
        ...
        tee.terminal.write(
            ...
            f"F/P = {force_per_perimeter_now:.4f}, "
...
with torch.no_grad():
        ...
        force_per_perimeter_final = forceTotal_final / perimeter_final
        final_force_ok = (True if _force_floor is None
                          else satisfies_force_floor(forceTotal_final, _force_floor))
        final_radial_ok = satisfies_radial_constraint(model, run_mode, max_expansion_ratio)
        ...
        scipy.io.savemat(
            mat_filename,
            {
                ...
                'forcePerPerimeterInitial': np.float64(force_per_perimeter_initial.item()),
                'forcePerPerimeterFinal':   np.float64(force_per_perimeter_final.item()),
                'seaInitial':     np.float64(force_per_perimeter_initial.item()),  # legacy compatibility
                'seaFinal':       np.float64(force_per_perimeter_final.item()),    # legacy compatibility
            }
        )
...
fpp_change_pct   = ((force_per_perimeter_final.item() - force_per_perimeter_initial.item()) / force_per_perimeter_initial.item()) * 100
...
print("  {:<8} {:>12.4f} {:>12.4f} {:>+9.2f}%".format(
    "F/P", force_per_perimeter_initial.item(), force_per_perimeter_final.item(), fpp_change_pct))
print(f"  ---")
