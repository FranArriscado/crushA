...
#                  optimizationModule_patched_v6.py
...
class crushOptimizerModel(nn.Module):
	...
	def forward(self):
		...
		# nodeLength[j] = half the sum of the two edges adjacent to node j
		# matches MATLAB layersProp: nodeLength(j) = sum(edgeLength(idxNodeRows))/2
...
# FIX 4 (v6): now returns edgeLengths as well as perimeter so that
# forward() can compute per-node nodeLength without recomputing edges.
# edgeLengths[j] = length of edge from node j to node j+1 (closed loop)
def perimeterCalc(orderedNodes):
    nodes_shifted = torch.roll(orderedNodes, -1, dims=0)
    diffs = nodes_shifted - orderedNodes
    edgeLengths = torch.norm(diffs, dim=1)  # [N]
...
# Hyperparameters
n_steps = 5000 # Training loop iterations
...
with torch.no_grad():
        ...
        # Build connectivity — consecutive ordered pairs, 1-based to match input format
        n = orderedNodes_final_np.shape[0]
        connectivity_final = np.column_stack([np.arange(1, n + 1), np.arange(2, n + 2)])
        connectivity_final[-1, 1] = 1  # close the loop: last node connects back to first
