#
#                  optimizationModule_patched_v2.py
#                   
#                     Tiago Cardoso
#                      up202006481
#                      FEUP, 2025
...
# SAFE 3: keep a snapshot of the best (lowest-loss) state so we can
#          roll back if NaN occurs.
best_loss = float('inf')
...
for epoch in range(n_steps):
        ...
        # SAFE 3: NaN detection — rollback to best state and stop
        if torch.isnan(loss) or torch.isinf(loss):
        	print(f"\n*** NaN/Inf detected at epoch {epoch}. "
        	      f"Rolling back to best state from epoch {best_epoch}. ***\n")
