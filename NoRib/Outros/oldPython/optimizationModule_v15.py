...
#==========================================================================
# ---- Plot ---- #
def plot_initial_and_final(initial_nodes, final_nodes, save_path="optim_result.pdf", box_dims=None):
    plt.rcParams.update({
        "text.usetex": True,
        "font.family": "serif",
    ...
    if box_dims is not None:
        ...
        rect = plt.Rectangle((-hw, -hh), box_dims[0], box_dims[1],
            ...
            linestyle='--', label=r'Box constraint')
        ax.add_patch(rect)
    ax.set_xlabel(r"$x$")
    ax.set_ylabel(r"$y$")
    ax.set_aspect('equal')
    plt.subplots_adjust(bottom=0.25)
    ax.legend(loc='upper center', bbox_to_anchor=(0.5, -0.25), ncol=3 if box_dims else 2, frameon=False)
...
def compute_loss(mode, force_total, perimeter, model,
    ...
    if mode == 'maxCF':
     ...
     loss = (-sea
        ...
        # Maximise SEA within a user-defined bounding box.
        # Box is centered at the initial centroid.
        # Nodes outside the box are penalised quadratically.
        sea = force_total / perimeter
        force_penalty = torch.relu(force_initial - force_total) ** 2

        # Reconstruct Cartesian coordinates (relative to centroid)
        ...
        # Penalise nodes outside the box
        ...
        raise ValueError(f"Unknown mode '{mode}'. Use 'maxCF', 'minP', 'maxSEA', or 'boxSEA'.")
...
# Box dimensions in mm, centered at the cross-section centroid.
# The optimizer will reshape the cross-section to maximise SEA
# while keeping all nodes inside this box.
...
lambda_crushForce = 1e6             # minP, maxSEA, boxSEA: force floor constraint
lambda_radialDist = 1e6             # maxCF and maxSEA: radial bounds
lambda_box = 1e6                    # boxSEA: bounding box constraint
...
if run_mode == 'boxSEA':
    print(f"  Box constraint:     {box_width:.1f} × {box_height:.1f} mm")
