...
def compute_loss(mode, force_total, perimeter, model,
    ...
    max_contraction_ratio : float
        Max allowed inward contraction ratio. The lower bound is
    ...
    if mode == 'maxCF':
        # Bidirectional but bounded: allow only limited inward
        # redistribution while keeping outward exploration available.
        # Also enforce a force floor so inward motion is only accepted
        # if force does not drop below the initial value.
...
# (relative) over the last conv_patience epochs.
conv_patience = 1000                 # epochs to wait for improvement
conv_tol = 1e-3                     # relative improvement threshold (0.01%)
...
# Max outward radial movement (maxCF and maxSEA modes)
max_expansion_ratio = 1.3
# Max inward movement allowed for maxCF in v18.
# Keep this much tighter than max_expansion_ratio so maxCF stays close
# to the original outward-driven behaviour of v17 by default.
...
parser.add_argument('--max-contraction', type=float, default=None,
                    help='Max inward contraction ratio for maxCF in v18.')
...
_tmp_base = f"{_shape}_{run_mode}_running_{run_timestamp}_(v{script_version})"

log_filename = str(RESULTS_NORIB / f"{_tmp_base}.txt")
mat_filename = str(RESULTS_NORIB / f"{_tmp_base}.mat")
pdf_filename = str(RESULTS_NORIB / f"{_tmp_base}.pdf")
...
# Early stopping state: track the objective (not loss)
best_objective = float('-inf')  # maximise by convention (negate for minP)
best_objective_r = model.r.data.clone()
best_objective_epoch = 0
...
for epoch in range(n_steps):
        ...
        if run_mode == 'maxCF':
            current_objective = force_total.item()
        elif run_mode == 'minP':
            current_objective = -perimeter.item()  # negate so "higher = better"
        elif run_mode in ('maxSEA', 'boxSEA'):
            current_objective = sea_now

        # Check if objective improved by at least conv_tol (relative)
        if best_objective == float('-inf') or \
...
_final_base = f"{_shape}_{run_mode}_{run_timestamp}_(v{script_version})"

log_filename_final = str(RESULTS_NORIB / f"{_final_base}.txt")
mat_filename_final = str(RESULTS_NORIB / f"{_final_base}.mat")
pdf_filename_final = str(RESULTS_NORIB / f"{_final_base}.pdf")
...
# ---- Stopping reason ---- #
if nan_detected:
    stop_reason = f'NaN/Inf at epoch {final_epoch} (rolled back to {best_epoch})'
elif converged:
    stop_reason = f'Converged at epoch {final_epoch} (restored best objective from epoch {best_objective_epoch})'
else:
    stop_reason = f'Step limit reached ({n_steps}), restored best objective from epoch {best_objective_epoch}'
