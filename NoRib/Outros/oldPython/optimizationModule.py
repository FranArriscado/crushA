...
class crushOptimizerModel(nn.Module):
	...
	def forward(self):
		...
		# RoC calculation
		RoC = RoCCalc(splineDer1, splineDer2, self.orderedIndices)
		RoC = RoC.clamp_min(5)
...
def splines(nodesExt, orderedNodes, s, overlapCount) -> Tuple[torch.Tensor, torch.Tensor]:
	...
	sQuery = torch.linspace(s[overlapCount],
                     s[-(overlapCount+1)],
                     nOriginal)
...
# Smoothness term that penalizes large variations in the curvature
def smoothnessTerm(r):
    # Calculate the first derivative (change in radius) and second derivative (curvature)
    dr = r[1:] - r[:-1]
    curvature = dr[1:] - dr[:-1]  # Second derivative approximation
    # Penalize large curvature variations
...
log_filename = f"optimisation_log_ORIGINAL_{run_timestamp}.txt"
...
print("\n" + "="*60)
print("CONVERGENCE STUDY — RUN SUMMARY (ORIGINAL)")
print("="*60)
