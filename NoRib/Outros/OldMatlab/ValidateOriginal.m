...
% Python final force to compare against (from the optimisation log .txt)
...
if abs(diff_pct) < 5
    fprintf('  Within 5%% -- acceptable for spline-type residual.\n');
elseif abs(diff_pct) < 10
    fprintf('  5-10%% -- check curvature equation and flat_transition.\n');
else
    fprintf('  > 10%% -- significant discrepancy. Investigate further.\n');
end
...
save(fullfile(cfg.validation, 'validation_diagnostics.mat'), ...
    'orderedNodes', 'RoC', 'crushStress', ...
    'nodeLength', 'forceNode', 'forceTotal', 'sQuery');
