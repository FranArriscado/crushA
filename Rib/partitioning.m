...
%==========================================================================
%==========================================================================
function layers = partitioning(FEModelData, elements, impactor, input)

% Variables initialization
% cross-section resultant from the intersection between impactor surface
% and FE Model surface
layers = struct( ...
    'NodesCoordinates',[],... % coordinates of each intersected node
    'ConnectivityList',[],... % connectivity list of the layer (edges of cross-section)
    'ElementsIntsec',[]); % ID of elements intersected (ID corresponding to 'elements.ConnectivityList' rows)

% Unpacking data
crushPlaneNormalVec = impactor.NormalVec;
PartitionsOffset = input.PartitionsOffset;
crushDirectionDef = input.crushDirectionDef;
directionVec = input.directionVec;
k=0; % impactor displacement increments
hasBeenIntersected = false; % mark that intersection has been 1 at least once
% change nomenclature only to be compatible with 'SurfaceIntersection' function
% plane --> impactor
% surface --> converted FE Model
plane.vertices = impactor.NodesCoordinates;
plane.faces = impactor.ConnectivityList;
surface.vertices = FEModelData.Nodes.Coordinates;
surface.faces = elements.ConnectivityList;

% set crush diretion: user-defined or crush plane normal vector
if crushDirectionDef == false
    directionVec = crushPlaneNormalVec;
end

while true

    k = k+1; % add 1 increment
    
    % compute intersection
    [intsec, intsurf] = SurfaceIntersection(surface, plane);

    % store intersection information in structure
    layers(k).NodesCoordinates = intsurf.vertices;
    layers(k).ConnectivityList = intsurf.edges;
  
    % obtain intersected elements (ID corresponding to 
    % 'elements.ConnectivityList' rows)
    [ElementsIntsec, ~, ~] = find(intsec);
    layers(k).ElementsIntsec = ElementsIntsec;

    % Store structure information of each layer in temporary variables (speed improvement)
    NodesCoords = layers(k).NodesCoordinates;
    ConnectList = layers(k).ConnectivityList;
    
    numNodes = size(layers(k).NodesCoordinates, 1);

    % check if intersection exists
    if numNodes == 0
        intersection = 0; % intersection does not exist
    else
        intersection = 1; % intersection exists

        % treat layers.NodesCoordinates to account for numeric imprecisions
        
        idxNew = (1:numNodes)';
        keep = true(numNodes, 1); % tracks which rows to keep
        for i=1:numNodes
            if ~keep(i)
                continue; % skip rows already marked as duplicates
            end
            diffs = NodesCoords(i+1:end,:) - NodesCoords(i,:);
            dist = sqrt(sum(diffs.^2,2));
            idxDuplicate = find(dist < 10^-4);
            % mark duplicates for removel and remap their indices
            for j = idxDuplicate'
                idxReal = i+j;
                keep(idxReal) = false; % mark duplicate row for removal
                idxNew(idxReal) = i;
            end
        end
        % get cleaned-up nodes coordinates list
        layers(k).NodesCoordinates = NodesCoords(keep, :);
        % build a map from old indices to new ones
        old_to_new = zeros(numNodes, 1);
        newID = 1;
        for i = 1:numNodes
            if keep(i)
                old_to_new(i) = newID;
                ...
                old_to_new(i) = old_to_new(idxNew(i));  % map to its retained duplicate
            end
        end
        % Update the connectivity list (edges) using new node indices
        ConnectList = old_to_new(ConnectList);

        %treat layers.ConnectivityList to account for numeric imprecisions
        row_uniformity = all(ConnectList == ConnectList(:,1), 2);
        ConnectList = ConnectList(~row_uniformity, :);
        [~,idx] = unique(ConnectList, 'rows', 'stable');

        layers(k).ConnectivityList = ConnectList(sort(idx), :);
    end

    % break cycle if end of the FE Model is reached 
    % (previously intersected and no intersection in the current increment)
    if hasBeenIntersected && intersection == 0
        break;
    end
    % mark that FE Model has been intersected
    if intersection == 1
        hasBeenIntersected = true;
    end
    
    % break cycle if no intersection is found after 100 increments
    if hasBeenIntersected == false && k>100
        error('No intersection found between impactor and FE model after 100 increments. Check crush direction vector.')
    end
    
    % update impactor coordinates for next intersection
    plane.vertices = plane.vertices + PartitionsOffset * directionVec;

end

% remove last layer (not intersected)
layers(end) = [];

% remove first layer if only one node is present
if size(layers(1).NodesCoordinates,1) == 1
    layers(1) = [];
end
