...
#
#  v8 changes:
#    - Uniform wall resampling now targets a perimeter-based node count:
#      N = round(resample_target_perimeter_scale * P +
#                resample_target_offset)
#      where P is the cleaned wall perimeter in mm.
#    - Added guardrails so the target count cannot undershoot the minimum
#      needed for a closed loop or the preserved landmark segments.
#
#  v7 changes:
...
def _single_role_thickness(values, role_name, decimals=6):
    ...
    if len(uniq) != 1:
        raise ValueError(
            ...
            f"{uniq.tolist()}. v8 expects a single {role_name} thickness."
        )
    return float(uniq[0])
...
if 'thickness' in data:
    ...
    if len(_unique_t) >= 5:
        raise ValueError(
            ...
            f"{_unique_t.tolist()}. v8 currently supports wall, rib, and up to "
