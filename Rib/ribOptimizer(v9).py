...
#
#  v9 changes:
#    - Added a rib-axis penalty that keeps the optimized rib aligned with
#      the source rib direction, preventing junction pairs from drifting
#      sideways and creating a tilted rib in otherwise well-behaved layers.
#    - Added rib-axis deviation reporting to the terminal summary and MAT
#      output for easier diagnosis.
#
#  v7 changes:
...
# ---- Rib-axis alignment penalty ---- #
# Keeps the rib aligned with the initial source rib direction without
# forcing the rib to keep its original length or midpoint.
rib_axis_weight_ratio = {
    'maxCF':    5.0,
    'minP':     5.0,
    'maxFPP':   5.0,
    'boxMaxCF': 5.0,
...
resample_target_perimeter_scale = 0.20
resample_target_offset = 60.0
resample_target_min_nodes = 24
...
def closed_loop_perimeter_np(points: np.ndarray) -> float:
...
def perimeter_based_resample_target(points: np.ndarray,
                                    perimeter_scale: float,
                                    offset: float,
                                    min_nodes: int = 24) -> tuple[int, float]:
    perimeter = closed_loop_perimeter_np(points)
    target_count = int(round(perimeter_scale * perimeter + offset))
    target_count = max(target_count, int(min_nodes), 3)
    return target_count, perimeter
...
def resample_closed_loop_uniform(points, target_count=None,
    ...
    target_count = max(int(round(target_count)), 3)
    ...
    target_count = max(target_count, n_segments)
...
class crushRibOptimizerModel(nn.Module):
    ...
    def __init__(self,
        ...
        initial_junc0 = wall_nodes[self.j0_wall_idx]
        initial_junc1 = wall_nodes[self.j1_wall_idx]
        initial_rib_vec = initial_junc1 - initial_junc0
        initial_rib_length = torch.linalg.norm(initial_rib_vec).clamp(min=1e-8)
        self.register_buffer('initial_rib_axis', initial_rib_vec / initial_rib_length)
        self.register_buffer('initial_rib_length', initial_rib_length)
    ...
    def rib_axis_penalty(self) -> torch.Tensor:
        wall_xyz = self._reconstruct_wall_xyz()
        rib_vec = wall_xyz[self.j1_wall_idx] - wall_xyz[self.j0_wall_idx]
        proj = torch.dot(rib_vec, self.initial_rib_axis)
        perp_vec = rib_vec - proj * self.initial_rib_axis
        return torch.dot(perp_vec, perp_vec) / (self.initial_rib_length ** 2 + 1e-12)

    def rib_axis_deviation_deg(self) -> torch.Tensor:
        wall_xyz = self._reconstruct_wall_xyz()
        rib_vec = wall_xyz[self.j1_wall_idx] - wall_xyz[self.j0_wall_idx]
        rib_len = torch.linalg.norm(rib_vec).clamp(min=1e-8)
        cosang = torch.dot(rib_vec / rib_len, self.initial_rib_axis).clamp(-1.0, 1.0)
        return torch.rad2deg(torch.acos(torch.abs(cosang)))
...
def compute_loss(mode, force_total, perimeter, model,
    ...
    lambdas : dict
        Penalty weights: 'smooth', 'force', 'radial', 'box', 'rib_axis'.
    ...
    rib_axis_penalty = model.rib_axis_penalty()
    ...
    if mode == 'maxCF':
        ...
        loss = (-force_total
                ...
                + lambdas['rib_axis'] * rib_axis_penalty)
...
if resample_uniform:
    target_wall_nodes, wall_perimeter_clean = perimeter_based_resample_target(
        wall_nodes_clean,
        perimeter_scale=resample_target_perimeter_scale,
        offset=resample_target_offset,
        min_nodes=resample_target_min_nodes,
    )
    wall_nodes_final, landmark_map = resample_closed_loop_uniform(
        wall_nodes_clean,
        target_count=target_wall_nodes,
    ...
    print("Uniform resampling target: "
          f"N = round({resample_target_perimeter_scale:.3f} * "
          f"{wall_perimeter_clean:.2f} + {resample_target_offset:.1f}) "
          f"= {target_wall_nodes} nodes")
...
with torch.no_grad():
    ...
    rib_axis_dev_init_deg = model.rib_axis_deviation_deg()
    ...
    print(f"Rib axis dev.:   {rib_axis_dev_init_deg.item():.4f} deg")
...
lambda_rib_axis = rib_axis_weight_ratio[run_mode] * objective_scale
print(f"\nLambda smoothness (auto): {lambda_smoothness:.2f}  "
      f"(ratio={smooth_weight_ratio[run_mode]} x objective={objective_scale:.2f})")
print(f"Lambda rib-axis (auto):   {lambda_rib_axis:.2f}  "
      f"(ratio={rib_axis_weight_ratio[run_mode]} x objective={objective_scale:.2f})")
...
with torch.no_grad():
    ...
    rib_axis_dev_final_deg = model.rib_axis_deviation_deg()
    ...
    savemat(mat_filename, {
        ...
        'ribAxisDeviationInitialDeg': np.float64(rib_axis_dev_init_deg.item()),
        'ribAxisDeviationFinalDeg':   np.float64(rib_axis_dev_final_deg.item()),
    })
...
print(f"  Thickness:          wall={wall_thickness_val:.4f}, "
      ...
      f"rib={rib_thickness_val:.4f} mm")
print(f"  Rib axis dev.:      {rib_axis_dev_final_deg.item():.4f} deg "
      f"(init {rib_axis_dev_init_deg.item():.4f} deg)")
if run_mode == 'boxMaxCF':
